...
def sum_atleast(ls,num):
    ...
    #     add = add+temp[x]
    # return (add>=num)
    return PbGe([(x,1) for x in ls],num)
...
s.add( And(base_cons) )
p = [Bool("p__{}".format(i)) for i in range(1000)]
...
def add_a_guess_solution( guess, reds, whites,step=1):
    ...
    if reds==0 and whites==0:
            ...
            for x in no_where:
                no_case.append(vs[x][guess[x]]==False)
            no_case = And(no_case)
            s.add(Implies(p[step],no_case))

    s.add( Implies(p[step],guess_cons ))
...
def play_game():
    ...
    while red < k:
        ...
        print(red,white)
        add_a_guess_solution( move, red, white,num_moves )
    print("Game solved!")
